"""
================================================================================
EduPPT Quality Analyzer Ultimate - Complete Edition v7.9.1 (Gemini 3 Support)
================================================================================
The MOST COMPREHENSIVE PowerPoint Quality Analyzer for B.Tech Education

FEATURES:
✅ ADDED: Google Gemini 3.0 Model Option (Preview)
...
✅ CRITICAL FIX: AUTO-CLEANUP of 'slide_images' folder (Prevents Disk Full)
...
Version: 7.9.1 (Gemini 3 + Auto-Cleanup + AWS Stability)
================================================================================
"""
...
class GeminiMultimodalAnalyzer:
    ...
    MODELS = {
        ...
        "gemini-3-flash-preview": "Gemini 3.0 Flash (Best - Recommended)",
        "gemini-3-pro-preview": "Gemini 3.0 Pro (Best - Recommended)",
        "gemini-2.5-flash": "Gemini 2.5 Flash (Best - Recommended)",
    }
...
class UltimateEduPPTAnalyzer:
    ...
    def _cleanup_generated_images(self):
        """Force delete all files in slide_images directory to save disk space"""
        try:
            ...
            logger.info("🧹 Slide images directory cleaned up.")
        except Exception as e:
    ...
    def run_complete_analysis(self, file_path: Path,
        ...
        # --- FINAL CLEANUP BLOCK (CRITICAL FIX) ---
        try:
            # 1. Remove the uploaded PPTX
            if file_path.exists():
                os.remove(file_path)

            # 2. Force Clean Memory
            clean_memory()

            # 3. WIPE THE SLIDE IMAGES FOLDER
...
*Report generated by EduPPT Quality Analyzer Ultimate v7.9.1*
...
def cleanup_temp_files():
    """Wipe data and slide_images directories on startup to prevent accumulation"""
    folders_to_clean = [Path("data"), Path("slide_images"), Path("cache")]
        ...
        if folder.exists():
            for file in folder.glob("*"):
                try:
                    if file.is_file():
                        file.unlink()
                        # Only log if it's not a hidden file to reduce noise
                        if not file.name.startswith('.'):
                            logger.info(f"🧹 Startup cleanup: Removed {file.name} from {folder.name}")
                except Exception as e:
...
def main():
    ...
    <div style="text-align: center; color: gray; padding: 2rem; 
                ...
                border-radius: 15px;">
        <h4>🎓 EduPPT Quality Analyzer Ultimate v7.9.1</h4>
